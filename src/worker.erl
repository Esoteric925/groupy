...
join(Id, Cast) ->
  receive
    {view, _} ->
      %erlang:display("kmr jag in hit?"), erlang:display(self()),
      Ref = make_ref(),
...
init_cont(Id, Rnd, Cast, Color, Sleep) ->
 % erlang:display("Kommer jag in i init_cont som slav"), erlang:display(self()),
  random:seed(Rnd, Rnd, Rnd),
  Title = "Worker: " ++ integer_to_list(Id),
  Gui = gui:start(Title, self()),
  %erlang:display("Har jag startat ett GUI"), erlang:display(self()),
